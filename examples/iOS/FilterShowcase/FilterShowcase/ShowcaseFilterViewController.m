...
//    videoCamera = [[GPUImageVideoCamera alloc] initWithSessionPreset:AVCaptureSessionPreset640x480 cameraPosition:AVCaptureDevicePositionFront];
    ...
    switch (filterType)
    {
        case TEST:
        {
            ...
            [self.filterSettingsSlider setValue:50.0];
            [self.filterSettingsSlider setMinimumValue:0.0];
            [self.filterSettingsSlider setMaximumValue:100.0];
        }
        case GPUImage_OLDPHOTO:
        {
            self.title = @"old";
            self.filterSettingsSlider.hidden = NO;
            ...
            filter = [[GPUImageOldPhotoStyleDateFilter alloc] init];
        } break;
            
        case GPUIMAGE_PIXELATION:
        {
            self.title = @"pixel";
            self.filterSettingsSlider.hidden = NO;
            ...
            filter = [[GPUImagePixelationFilter alloc] init];
        }
            break;
        case GPUIMAGE_SEPIA:
        {
...
- (IBAction)updateFilterFromSlider:(id)sender;
{
    ...
    [self.slideVal setText:[NSString stringWithFormat: @"%f", v]];
    [videoCamera resetBenchmarkAverage];
    switch(filterType)
    {
        //case GPUImage_OLDPHOTO :[(GPUImage_OL)]
            
        case GPUIMAGE_PIXELATION :[(GPUImagePixelationFilter*)filter setTexelScale:v];
            break;
...
//        case GPUIMAGE_VIGNETTE: [(GPUImageVignetteFilter *)filter setVignetteEnd:[(UISlider *)sender value]]; break;
        case GPUIMAGE_VIGNETTE: [(GPUImageVignetteFilter *)filter setVignetteStart:[(UISlider *)sender value]]; break;
